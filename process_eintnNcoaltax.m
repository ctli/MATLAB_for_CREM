...
figure(5); hold on;
tmp = [fosegycons_n,nhw_consumption_n,-ele_trade_n];
hb = bar(yr, tmp, b, 'stacked', 'edge','none');
set(hb(1), 'facec', [0.8 0.8 0.8], 'edge','none');
set(hb(2), 'facec', [0.675 1 0.675], 'edge','none');
set(hb(3), 'facec', 'none', 'edge','k');
bar(yr, sum(tmp,2), b, 'facec', 'none', 'edge', 'r', 'linewidth', 1);
...
set(gca, 'xminortick', 'on');
set(gca, 'pos', [0.08    0.6875    0.75    0.25]);
title('Regional Coal Share (w/ National Energy Intensity Target)');
my_gridline;
...
%% policy scenario: regional coal tax
...
% ===========================
figure(1); hold on;
plot(yr, egy_intensity_n, 'o-', 'markersize', 5, 'color', [0 0.7 0]);
legend('No Policy', 'w/ National Energy Intensity Target', 'Coal Tax in 3 Key Regions', 3);
% my_gridline; export_fig egyintn -painters;

% ==========
figure(2); hold on;
plot(yr, col_share_n, 'o-', 'markersize', 5, 'color', [0 0.7 0]);
% my_gridline; export_fig coal_share -painters;

% ==========
figure(3); hold on;
plot(yr, nhw_share_n, 'o-', 'markersize', 5, 'color', [0 0.7 0]);
% my_gridline; export_fig nhw_share -painters;

% ==========
figure(4); hold on;
plot(yr, GDP_n, 'o-', 'markersize', 5, 'color', [0 0.7 0]);
...
set(hb(2), 'facec', [0.675 1 0.675], 'edge','none');
set(hb(3), 'facec', 'none', 'edge','k');
bar(yr+dx, sum(tmp,2), b, 'facec', 'none', 'edge', [0 0.7 0], 'linewidth', 1);

text(yr(1)-dx, sum(tmp(1,:))+100, 'Left: No Policy', 'fontsize', 6, 'rotation', 90, 'color', 'b');
text(yr(1), sum(tmp(1,:))+100, 'Middle: w/ National Egy. Int. Target', 'fontsize', 6, 'rotation', 90, 'color', 'r');
text(yr(1)+dx, sum(tmp(1,:))+100, 'Right: Plus Coal Tax in 3 Key Regions', 'fontsize', 6, 'rotation', 90, 'color', [0 0.7 0]);
...
% ==========
figure(8); clf
set(gcf, 'unit', 'inch', 'pos', [26.2354    5.5938    5.8333    4.3750]);

subplot(3,1,1);
plot(yr, col_consumption(:,JJJ), 'o-', 'markersize', 5);
set(gca, 'fontsize', 8);
ylabel('JJJ');
legend(r(JJJ), 'location', 'eastoutside');
ylim([0 400]);
set(gca, 'yminortick', 'on', 'xminortick', 'on');
set(gca, 'pos', [0.08    0.6875    0.75    0.25]);
title('Regional Coal Consumption (mtce) (w/ National Energy Intensity Target + Regional Coal Tax)');
my_gridline;

subplot(3,1,2);
plot(yr, col_consumption(:,YRD), 'o-', 'markersize', 5);
set(gca, 'fontsize', 8);
ylabel('YRD')
legend(r(YRD), 'location', 'eastoutside');
ylim([0 400]);
...
subplot(3,1,3);
plot(yr, col_consumption(:,PRD), 'o-', 'markersize', 5);
set(gca, 'fontsize', 8);
ylabel('PRD');
legend(r(PRD), 'location', 'eastoutside');
ylim([0 400]);
...
% export_fig regional_coal_consumption_wCoalTax -painters;

% ==========
figure(80); clf;
set(gcf, 'unit', 'inch', 'pos', [26.2354    0.2292    5.8333    4.3750]);

subplot(3,1,1);
plot(yr, col_share(:,JJJ), 'o-', 'markersize', 5);
set(gca, 'fontsize', 8);
ylabel('JJJ');
legend(r(JJJ), 'location', 'eastoutside');
ylim([0 1]);
...
set(gca, 'xminortick', 'on');
set(gca, 'pos', [0.08    0.6875    0.75    0.25]);
title('Regional Coal Share (w/ National Energy Intensity Target + Regional Coal Tax)');
my_gridline;

subplot(3,1,2);
plot(yr, col_share(:,YRD), 'o-', 'markersize', 5);
set(gca, 'fontsize', 8);
ylabel('YRD')
legend(r(YRD), 'location', 'eastoutside');
ylim([0 1]);
...
subplot(3,1,3);
plot(yr, col_share(:,PRD), 'o-', 'markersize', 5);
set(gca, 'fontsize', 8);
ylabel('PRD');
legend(r(PRD), 'location', 'eastoutside');
ylim([0 1]);
...
% ==========
figure(20); hold on;
bar(1:30, col_consumption(end,:), 0.4, 'facec', [0 0.6 0], 'edgecolor', 'none');
legend('No Policy', 'w/ National Energy Intensity Target',2, 'orientation', 'horizontal');
