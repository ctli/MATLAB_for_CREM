...
figure(14); clf;
plot(yr, co2_n/1e3, '^-', 'color', [0.5 0.5 0.5], 'markersize', 5);
set(gca, 'fontsize', 8);
ylabel('CO2 (Billion tons)', 'Fontsize', 9);
set(gcf, 'unit', 'inch', 'pos', [0.25+4.15*3    4.8229    4.0000    3.0000]);
% for yn = 3:length(yr)
%     text(yr(yn), co2_n(yn)/1e3+0.5, num2str(co2_n(yn)/1e3, '%2.1f'), 'fontsize', 8, 'horizontalalignment', 'center');
% end

figure(15); clf;
plot(yr, GDP_n, 'r^-', 'color', [0.5 0.5 0.5], 'markersize', 5);
set(gca, 'fontsize', 8);
...
figure(14); hold on;
plot(yr, co2_n/1e3, 'go-', 'color', [0 0.8 0], 'markersize', 5);

figure(15); hold on;
plot(yr, GDP_n, 'go-', 'color', [0 0.8 0], 'markersize', 5);
...
figure(14); hold on;
plot(yr(1:length(co2_n)), co2_n/1e3, 'rx-', 'markersize', 5);
